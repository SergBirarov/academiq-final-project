...
Begin 
	Declare assignment_cursor cursor for 
		Select AssignmentId, Deadline
		From Assignments
		Where Deadline < GETDATE() And IsVisible = 1
		And Not Exists(
			Select 1
			From Assignment_Of_Student
			Where Assignments.AssignmentId = Assignment_Of_Student.AssignmentId And Submitted = 1
			);
		OPEN assignment_cursor;

    DECLARE @AssignmentId INT, @Deadline DATETIME;

    FETCH NEXT FROM assignment_cursor INTO @AssignmentId, @Deadline;

    WHILE @@FETCH_STATUS = 0
    BEGIN
        -- here will be added the logic for an overdue assignment (either using dbmail() or our notification API
        PRINT 'Assignment ' + CAST(@AssignmentId AS NVARCHAR) + ' is past the deadline and not submitted.';
...
EXEC Check_Overdue_Assignments;
go



CREATE PROCEDURE UnassignedLecturers
AS
BEGIN
    DECLARE unassignedLecturersCursor CURSOR FOR
        SELECT L.LecturerId, L.FirstName, L.LastName
        FROM Lecturers L
        LEFT JOIN Lecturers_In_Course LIC ON L.LecturerId = LIC.LecturerId
        WHERE LIC.LecturerId IS NULL
        AND LIC.School_Year = YEAR(GETDATE()) + 1;

    DECLARE @LecturerId INT, @FirstName NVARCHAR(50), @LastName NVARCHAR(50);

    OPEN unassignedLecturersCursor;

    FETCH NEXT FROM unassignedLecturersCursor INTO @LecturerId, @FirstName, @LastName;

    WHILE @@FETCH_STATUS = 0
    BEGIN
        PRINT 'Lecturer ID: ' + CAST(@LecturerId AS NVARCHAR(10)) +
              ', Name: ' + @FirstName + ' ' + @LastName;

        FETCH NEXT FROM unassignedLecturersCursor INTO @LecturerId, @FirstName, @LastName;
    END;

    CLOSE unassignedLecturersCursor;
    DEALLOCATE unassignedLecturersCursor;
END;
GO

EXEC UnassignedLecturers;
...
BEGIN
    ...
    BEGIN TRY
		...
		SET @command = 'bcp "SELECT * FROM ' + db_name() + '.dbo.'+ @table_name + '" queryout "' + @file_name + '"' + ' -c -t, -T -S SERGEYSG-15\SQLEXPRESS';
